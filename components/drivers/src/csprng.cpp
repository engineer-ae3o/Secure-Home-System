...
namespace rnd {
    ...
    namespace {
        ...
        volatile uint32_t g_rtc_jitter{};
        ...
        std::array<uint16_t, ADC_NUM_CHANNELS> g_adc_buf{};

        // Stores the boot cycle count
        uint32_t g_boot_cycle_count{};

        // Tasks TCBs and Stacks
        constexpr uint32_t ENTROPY_TASK_STACK_BYTES = 512;

        TaskHandle_t g_entropy_task_handle{};
        StaticTask_t g_entropy_task_tcb{};

        std::array<StackType_t, utils::bytes_to_words(ENTROPY_TASK_STACK_BYTES)> g_entropy_task_stack{};
        ...
        public:
            ...
            scoped_mutex_t() {
                xSemaphoreTakeRecursive(g_task_mutex, portMAX_DELAY);
            }
        ...
        uint8_t get_entropy_mixture() {
            ...
            entropy ^= xor_bytes_in_word(g_rtc_jitter);

            // Pack the 4 LSBs of all the ADC samples before use
            uint32_t packed_adc_samples{};
            for (uint32_t shift{}; const auto& sample : g_adc_buf) {
            ...
            entropy ^= xor_bytes_in_word(g_boot_cycle_count);
    ...
    utils::error_t init() {
        ...
        TRY_HAL(HAL_DMA_Init(&g_dma_handle));
        ...
        g_adc_handle.Init.NbrOfConversion       = ADC_NUM_CHANNELS;
        TRY_HAL(HAL_ADC_Init(&g_adc_handle));
        ...
        TRY_HAL(HAL_ADC_Start_DMA(&g_adc_handle, reinterpret_cast<uint32_t*>(g_adc_buf.data()), ADC_NUM_CHANNELS));
        ...
        g_rtc_handle.Init.AsynchPrediv = 39; // RTC clock frequency of 40kHz / (39  + 1) = 1kHz
        g_rtc_handle.Init.OutPut       = RTC_OUTPUTSOURCE_NONE;
        TRY_HAL(HAL_RTC_Init(&g_rtc_handle));

        TRY_HAL(HAL_RTCEx_SetSecond_IT(&g_rtc_handle));
        ...
        g_boot_cycle_count = ret.value();

        if (g_boot_cycle_count == 0) {
        ...
        g_entropy_task_handle = xTaskCreateStatic(entropy_task,
                                                  "Entropy Task",
                                                  utils::bytes_to_words(ENTROPY_TASK_STACK_BYTES),
                                                  {},
                                                  1,
                                                  g_entropy_task_stack.data(),
                                                  &g_entropy_task_tcb);
    ...
    utils::error_t deinit() {
...

        {
            ...
            TRY_HAL(HAL_ADC_Stop_DMA(&g_adc_handle));
            TRY_HAL(HAL_DMA_DeInit(&g_dma_handle));
            TRY_HAL(HAL_ADC_DeInit(&g_adc_handle));
            TRY_HAL(HAL_RTC_DeInit(&g_rtc_handle));

            // Delete the task before deletion of the mutex
            vTaskDelete(g_entropy_task_handle);
...
extern "C" {
    ...
    void HAL_RTCEx_RTCEventCallback(RTC_HandleTypeDef* hrtc) {
        ...
        rnd::g_rtc_jitter = TIM2->CNT;
    }

    void RTC_IRQHandler() {
        HAL_RTCEx_RTCIRQHandler(&rnd::g_rtc_handle);
    }
