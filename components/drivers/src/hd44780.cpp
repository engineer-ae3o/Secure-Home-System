...
namespace lcd {
    ...
    void init() {
        ...
        send_nibble(0x3U, 0U); // Function set 1
        vTaskDelay(pdMS_TO_TICKS(5));
        send_nibble(0x3U, 0U); // Function set 2
        vTaskDelay(pdMS_TO_TICKS(1));
        send_nibble(0x3U, 0U); // Function set 3
        vTaskDelay(pdMS_TO_TICKS(1));
        send_nibble(0x2U, 0U); // 4 bit mode
        vTaskDelay(pdMS_TO_TICKS(1));

        // Full function set
        send_cmd(0x28U); // 4 bit, 2 lines, 5x8 dots
        vTaskDelay(pdMS_TO_TICKS(2));
        send_cmd(0x08U); // Display off
        vTaskDelay(pdMS_TO_TICKS(2));
        send_cmd(0x01U); // Display clear
        vTaskDelay(pdMS_TO_TICKS(2));
        send_cmd(0x06U); // Entry mode
        vTaskDelay(pdMS_TO_TICKS(2));
        send_cmd(0x0CU); // Display on
        vTaskDelay(pdMS_TO_TICKS(2));
