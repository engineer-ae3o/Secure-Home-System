...
void init() {
    ...
    send_nibble(0x3U, 0U);
    vTaskDelay(pdMS_TO_TICKS(5)); // Function set 1
    send_nibble(0x3U, 0U);
    vTaskDelay(pdMS_TO_TICKS(1)); // Function set 2
    send_nibble(0x3U, 0U);
    vTaskDelay(pdMS_TO_TICKS(1)); // Function set 3
    send_nibble(0x2U, 0U);
    vTaskDelay(pdMS_TO_TICKS(1)); // 4 bit mode

    // Full function set
    send_cmd(0x28U);
    vTaskDelay(pdMS_TO_TICKS(2)); // 4 bit, 2 lines, 5x8 dots
    send_cmd(0x08U);
    vTaskDelay(pdMS_TO_TICKS(2)); // Display off
    send_cmd(0x01U);
    vTaskDelay(pdMS_TO_TICKS(2)); // Display clear
    send_cmd(0x06U);
    vTaskDelay(pdMS_TO_TICKS(2)); // Entry mode
    send_cmd(0x0CU);
    vTaskDelay(pdMS_TO_TICKS(2)); // Display on
