...
namespace file {
    ...
    namespace {
        ...
        constexpr uint32_t LOOKAHEAD_BUF_SIZE_BYTES = 8;
        constexpr uint32_t CACHE_SIZE_BYTES         = BLOCK_SIZE_BYTES / 8;

        // LittleFS buffers
        std::array<uint8_t, CACHE_SIZE_BYTES>         g_read_buffer{};
        std::array<uint8_t, CACHE_SIZE_BYTES>         g_prog_buffer{};
        std::array<uint8_t, LOOKAHEAD_BUF_SIZE_BYTES> g_lookahead_buffer{};
        ...
        // File caches
        std::array<std::array<uint8_t, CACHE_SIZE_BYTES>, std::to_underlying(name_t::COUNT)> g_file_cache{};

        // Lookup table for the files being used
        constinit std::array<file_t, std::to_underlying(name_t::COUNT)> g_file_lut = {{
            ...
            [std::to_underlying(name_t::COUNTER)] =
                {
                    ...
                    .file_config =
                        {
                            .buffer     = g_file_cache[std::to_underlying(name_t::COUNTER)].data(),
                            .attrs      = nullptr,
                            .attr_count = 0,
                        },
                },
            [std::to_underlying(name_t::PASSWORD)] =
                {
                    ...
                    .file_config =
                        {
                            .buffer     = g_file_cache[std::to_underlying(name_t::PASSWORD)].data(),
                            .attrs      = nullptr,
                            .attr_count = 0,
                        },
                },
            [std::to_underlying(name_t::PNUMBERS)] =
                {
                    ...
                    .file_config =
                        {
                            .buffer     = g_file_cache[std::to_underlying(name_t::PNUMBERS)].data(),
                            .attrs      = nullptr,
                            .attr_count = 0,
                        },
                },
            [std::to_underlying(name_t::ASCON_SEED)] =
                {
                    ...
                    .file_config =
                        {
                            .buffer     = g_file_cache[std::to_underlying(name_t::ASCON_SEED)].data(),
                            .attrs      = nullptr,
                            .attr_count = 0,
                        },
                },
        ...
        // Helper
        constexpr uint32_t page_idx_to_phy_addr(uint32_t idx) {
...
#define TRY_LFS(func, err)                                                                                                                 \
    ...
    utils::error_t init() {
        ...
        // Create the mutex
        g_task_mutex = xSemaphoreCreateMutexStatic(&g_task_mutex_buffer);
        ...
        static const lfs_config s_lfs_config = {
            ...
            .lookahead_size = LOOKAHEAD_BUF_SIZE_BYTES,
            .compact_thresh = 0,

            // Statically provided buffers
            .read_buffer      = g_read_buffer.data(),
            .prog_buffer      = g_prog_buffer.data(),
            .lookahead_buffer = g_lookahead_buffer.data(),
        ...
        // Setup filesystem
        bool first_boot{};
        int  ret = lfs_mount(&g_lfs_handle, &s_lfs_config);
        if (ret < 0) [[unlikely]] {
            // This error should happen only once, at first boot
            TRY_LFS(lfs_format(&g_lfs_handle, &s_lfs_config), utils::error_t::FILE_FS_FAILED_TO_FORMAT);
            TRY_LFS(lfs_mount(&g_lfs_handle, &s_lfs_config), utils::error_t::FILE_FS_FAILED_TO_MOUNT);
            first_boot = true;
        }

        // Open counter file. Create it if it doesn't yet exist
        TRY_LFS(lfs_file_opencfg(&g_lfs_handle,
                                 &g_file_lut[std::to_underlying(name_t::COUNTER)].file,
                                 g_file_lut[std::to_underlying(name_t::COUNTER)].file_path.data(),
                                 (LFS_O_CREAT | LFS_O_RDWR),
                                 &g_file_lut[std::to_underlying(name_t::COUNTER)].file_config),
                utils::error_t::FILE_FAILED_TO_OPEN);

        // Read counter from file
        ret = lfs_file_read(
            &g_lfs_handle, &g_file_lut[std::to_underlying(name_t::COUNTER)].file, &g_boot_cycle_counter, sizeof(g_boot_cycle_counter));
        if (ret != sizeof(g_boot_cycle_counter) && ret != 0) {
            return utils::error_t::FILE_FAILED_TO_READ;
        }

        // Rewind file pointer back to starting position
        TRY_LFS(lfs_file_rewind(&g_lfs_handle, &g_file_lut[std::to_underlying(name_t::COUNTER)].file), utils::error_t::FILE_FAILED_TO_SEEK);
        ...
        if (!first_boot) [[likely]] {
            g_boot_cycle_counter++;
        }

        ret = lfs_file_write(
            &g_lfs_handle, &g_file_lut[std::to_underlying(name_t::COUNTER)].file, &g_boot_cycle_counter, sizeof(g_boot_cycle_counter));
        if (ret != sizeof(g_boot_cycle_counter)) {
            return utils::error_t::FILE_FAILED_TO_WRITE;
        }

        // Close the counter file
        TRY_LFS(lfs_file_close(&g_lfs_handle, &g_file_lut[std::to_underlying(name_t::COUNTER)].file), utils::error_t::FILE_FAILED_TO_CLOSE);

        // Open password and phone number files. Create if it doesn't yet exist
        // Pnumbers file
        TRY_LFS(lfs_file_opencfg(&g_lfs_handle,
                                 &g_file_lut[std::to_underlying(name_t::PNUMBERS)].file,
                                 g_file_lut[std::to_underlying(name_t::PNUMBERS)].file_path.data(),
                                 (LFS_O_CREAT | LFS_O_RDWR),
                                 &g_file_lut[std::to_underlying(name_t::PNUMBERS)].file_config),
                utils::error_t::FILE_FAILED_TO_OPEN);

        // Password file
        TRY_LFS(lfs_file_opencfg(&g_lfs_handle,
                                 &g_file_lut[std::to_underlying(name_t::PASSWORD)].file,
                                 g_file_lut[std::to_underlying(name_t::PASSWORD)].file_path.data(),
                                 (LFS_O_CREAT | LFS_O_RDWR),
                                 &g_file_lut[std::to_underlying(name_t::PASSWORD)].file_config),
                utils::error_t::FILE_FAILED_TO_OPEN);

        // Ascon seed file
        TRY_LFS(lfs_file_opencfg(&g_lfs_handle,
                                 &g_file_lut[std::to_underlying(name_t::ASCON_SEED)].file,
                                 g_file_lut[std::to_underlying(name_t::ASCON_SEED)].file_path.data(),
                                 (LFS_O_CREAT | LFS_O_RDWR),
                                 &g_file_lut[std::to_underlying(name_t::ASCON_SEED)].file_config),
                utils::error_t::FILE_FAILED_TO_OPEN);
    ...
    utils::error_t deinit() {
...

        {
            ...
            TRY_LFS(lfs_file_close(&g_lfs_handle, &g_file_lut[std::to_underlying(name_t::PASSWORD)].file),
                    utils::error_t::FILE_FAILED_TO_CLOSE);

            TRY_LFS(lfs_file_close(&g_lfs_handle, &g_file_lut[std::to_underlying(name_t::PNUMBERS)].file),
                    utils::error_t::FILE_FAILED_TO_CLOSE);

            TRY_LFS(lfs_file_close(&g_lfs_handle, &g_file_lut[std::to_underlying(name_t::ASCON_SEED)].file),
                    utils::error_t::FILE_FAILED_TO_CLOSE);

            TRY_LFS(lfs_unmount(&g_lfs_handle), utils::error_t::FILE_FS_FAILED_TO_UNMOUNT);
        }
    ...
    std::expected<uint32_t, utils::error_t> get_boot_cycle_count() {
        ...
        return g_boot_cycle_counter;
    }

    utils::error_t write(name_t file, std::span<const uint8_t> data, uint32_t offset) {
        if (!g_is_initialized) {
            return utils::error_t::ERR_INVALID_STATE;
        }
        ...
        TRY_LFS(lfs_file_seek(&g_lfs_handle, &g_file_lut[std::to_underlying(file)].file, static_cast<int>(offset), LFS_SEEK_SET),
                utils::error_t::FILE_FAILED_TO_SEEK);

        auto ret = lfs_file_write(&g_lfs_handle, &g_file_lut[std::to_underlying(file)].file, data.data(), data.size());
    ...
    utils::error_t read(name_t file, std::span<uint8_t> data, uint32_t offset) {
        if (!g_is_initialized) {
            return utils::error_t::ERR_INVALID_STATE;
        }
        ...
        auto ret = lfs_file_read(&g_lfs_handle, &g_file_lut[std::to_underlying(file)].file, data.data(), data.size());
        if (ret != static_cast<int>(data.size())) {
            return utils::error_t::FILE_FAILED_TO_READ;
        }
    ...
    utils::error_t sync(name_t file) {
        ...
        TRY_LFS(lfs_file_sync(&g_lfs_handle, &g_file_lut[std::to_underlying(file)].file), utils::error_t::FILE_FAILED_TO_SYNC);
